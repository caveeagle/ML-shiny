# app.py — Shiny for Python form (all labels and comments in English)

from shiny import App, ui, render, reactive

app_ui = ui.page_fluid(
    ui.h3("Describe your property"),  # translated title
    ...
    ui.input_select(  # optional (server-side logic can enforce if needed)
        "property_type",
        "Property type:",
        choices=[("", ""), ("house", "House"), ("apartment", "Apartment")],
        selected=""
    ),
    ui.input_select(  # optional
        "property_subtype",
        "Property subtype:",
        choices=[
            ("", ""),
    ...
    ui.input_select(  # categorical with default "Fully equipped"
        "equipped_kitchen",
        "Equipped kitchen:",
        choices=[
    ...
    # facades_number: optional, empty by default; server-side default to 3 when None
    ui.input_numeric("facades_number", "Facades number:", value=None, min=1, max=4, step=1),
    # build_year: optional; server-side default to 2011 when None
    ui.input_numeric("build_year", "Build year:", value=None, min=1800, max=2024, step=1),
    # cadastral_income: optional, left for server-side handling
    ui.input_numeric("cadastral_income", "Cadastral income:", value=None, min=0, step=1),
    # primary_energy_consumption: optional, left for server-side handling
    ui.input_numeric(
...
def server(input, output, session):
    # Helper: collect inputs and apply server-side defaults for some optional fields
    ...
    def validate_required():
        ...
        if input.rooms() is None:
            errors.append("Rooms is required (1–15).")
        if input.area() is None:
            errors.append("Area is required (10–600).")
        if input.number_floors() is None:
            errors.append("Number of floors is required (1–10).")
        return errors

    @reactive.Effect
    @reactive.event(input.submit)
    def _on_submit():
        ui.notification_remove()  # clear previous notifications
